...
package gestao_riscos.stg_arq_riscos_dolar_0_1;
...
/**
 * Job: STG_ARQ_RISCOS_DOLAR Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class STG_ARQ_RISCOS_DOLAR implements TalendJob {
...
public String getSTG_USER(){
	...
	private final String jobName = "STG_ARQ_RISCOS_DOLAR";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				STG_ARQ_RISCOS_DOLAR.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(STG_ARQ_RISCOS_DOLAR.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR = new byte[0];
    static byte[] commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR = new byte[0];

	
			    public String BZFXPTAX;

				public String getBZFXPTAX () {
					return this.BZFXPTAX;
				}
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR.length) {
				if(length < 1024 && commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR.length == 0) {
   					commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR = new byte[1024];
				} else {
   					commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR, 0, length);
			strReturn = new String(commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR, 0, length);
			strReturn = new String(commonByteArray_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_GESTAO_RISCOS_STG_ARQ_RISCOS_DOLAR) {

        	try {

        		int length = 0;
		
					this.BZFXPTAX = readString(dis);
	...
	final String decryptedPassword_tDBOutput_2 = context.STG_PWD; 
        ...
        if(dbschema_tDBOutput_2 == null || dbschema_tDBOutput_2.trim().length() == 0) {
            ...
            try (java.sql.Statement stmtCreate_tDBOutput_2 = conn_tDBOutput_2.createStatement()) {
                stmtCreate_tDBOutput_2.execute("CREATE TABLE " + tableName_tDBOutput_2 + "(BZFXPTAX VARCHAR2(255)  ,PX_BID VARCHAR2(255)  ,PX_ASK VARCHAR2(255)  ,USDBRL_Curncy VARCHAR2(255)  ,DATAHORAPROCESSAMENTO VARCHAR2(255)  ," + "DATAHORAINCLUSAO" + " VARCHAR2(255))");
            }
                String insert_tDBOutput_2 = "INSERT INTO " + tableName_tDBOutput_2 + " (BZFXPTAX,PX_BID,PX_ASK,USDBRL_Curncy,DATAHORAPROCESSAMENTO," + "DATAHORAINCLUSAO" + ") VALUES (?,?,?,?,?," + "TO_CHAR(SYSDATE,'YYYY-MM-DD HH24:MI:SS')" + ")";    
 ...
 */
	...
	final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:zRjfUbzHpJuwQSL/4pqZNp7vPz5/lWW9Xx+fTQ==");
		...
		try {
		excelReader_tFileInputExcel_1.addSheetName("Dólar", false);
		...
		try{
			...
			if( temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim().length() > 0) {
				...
				curColName_tFileInputExcel_1 = "BZFXPTAX";

				row1.BZFXPTAX = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim();
			}else{
				row1.BZFXPTAX = null;
			...
			if( temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim().length() > 0) {
				curColNum_tFileInputExcel_1=columnIndex_tFileInputExcel_1 + start_column_tFileInputExcel_1 + 1;
				curColName_tFileInputExcel_1 = "PX_BID";

				row1.PX_BID = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim();
			}else{
				row1.PX_BID = null;
				emptyColumnCount_tFileInputExcel_1++;
			}
			...
			if( temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim().length() > 0) {
				curColNum_tFileInputExcel_1=columnIndex_tFileInputExcel_1 + start_column_tFileInputExcel_1 + 1;
				curColName_tFileInputExcel_1 = "PX_ASK";

				row1.PX_ASK = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim();
			}else{
				row1.PX_ASK = null;
				emptyColumnCount_tFileInputExcel_1++;
			}
			...
			if( temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim().length() > 0) {
				curColNum_tFileInputExcel_1=columnIndex_tFileInputExcel_1 + start_column_tFileInputExcel_1 + 1;
				curColName_tFileInputExcel_1 = "USDBRL_Curncy";

				row1.USDBRL_Curncy = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].trim();
			}else{
...
if(row1 != null) { 
    ...
    Operator_tFilterRow_1 ope_tFilterRow_1 = new Operator_tFilterRow_1("&&");
            ope_tFilterRow_1.matches((row1.BZFXPTAX == null? false : row1.BZFXPTAX.compareTo("") != 0)
                           , "BZFXPTAX.compareTo(\"\") != 0 failed");
    
    if (ope_tFilterRow_1.getMatchFlag()) {
              if(row2 == null){ 
                row2 = new row2Struct();
              }
               row2.BZFXPTAX = row1.BZFXPTAX;
...
// # Output table : 'row3'
row3_tmp.BZFXPTAX = row2.BZFXPTAX ;
row3_tmp.PX_BID = row2.PX_BID ;
row3_tmp.PX_ASK = row2.PX_ASK ;
row3_tmp.USDBRL_Curncy = row2.USDBRL_Curncy ;
...
if(row3 != null) { 
	...
	currentComponent="tDBOutput_2";
        ...
        whetherReject_tDBOutput_2 = false;
                        if(row3.BZFXPTAX == null) {
pstmt_tDBOutput_2.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(1, row3.BZFXPTAX);
}

                        if(row3.PX_BID == null) {
pstmt_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(2, row3.PX_BID);
}

                        if(row3.PX_ASK == null) {
pstmt_tDBOutput_2.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(3, row3.PX_ASK);
}

                        if(row3.USDBRL_Curncy == null) {
pstmt_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(4, row3.USDBRL_Curncy);
}

                        if(row3.DATAHORAPROCESSAMENTO == null) {
pstmt_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(5, row3.DATAHORAPROCESSAMENTO);
}
 ...
 */
    ...
    public static void main(String[] args){
        final STG_ARQ_RISCOS_DOLAR STG_ARQ_RISCOS_DOLARClass = new STG_ARQ_RISCOS_DOLAR();

        int exitCode = STG_ARQ_RISCOS_DOLARClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = STG_ARQ_RISCOS_DOLAR.class.getClassLoader().getResourceAsStream("gestao_riscos/stg_arq_riscos_dolar_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = STG_ARQ_RISCOS_DOLAR.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : STG_ARQ_RISCOS_DOLAR");
        }
...
/************************************************************************************************
 *     114330 characters generated by Talend Open Studio for Data Integration 
 *     on the 10 de dezembro de 2025 08:18:57 BRT
 ************************************************************************************************/